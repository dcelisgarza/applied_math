program test_fluid
  ...
  call FldSlv % init(names = names, whd = whd, rho = 1._dp, scale = 1._dp)
  ...
  call FldSlv % u(2) % UpdateVals
    ...
    print*, "PRHS = ", FldSlv % prhs

    ! GSPSlv2D works!!!!!!
    call FldSlv % GSPSlv2D(5, 1.D-5)
    print*, FldSlv % p


    ! Test add flow subroutine.
    call FldSlv % rho % AddFlow(Vec2D2([0.5_dp,1._dp,1._dp,1._dp]), 1._dp)

    print*, FldSlv % rho % old

    call FldSlv % WriteToFile(1)
    call FldSlv % WriteToFile(2)
    call FldSlv % WriteToFile(3)

    allocate(TestArray(4))
    open(unit=1, file="density.dat", access="stream", status="old")
    read(1) TestArray
    print*,"Density = ", TestArray
    close(1)

    open(unit=1, file="pressure.dat", access="stream", status="old")
    read(1) TestArray
    print*,"Pressure = ", TestArray
    close(1)

    deallocate(TestArray)
    allocate(TestArray(6))

    open(unit=1, file="velocity.dat", access="stream", status="old")
    do i = 1, 2
      read(1) TestArray
      print*,"Velocity = ",i , TestArray
    end do
    close(1)
